...
- **App shell** — two top-level tabs (`#appTabs`: **Roster & data** ↔ **Groups**) toggle `#view-roster` / `#view-groups` via `showView()`; a header **icon transport bar** (`.iconbtn`, aria-labelled) holds ✨ example / 📂 open / 📋 paste / ⬇ template / ↶↷ undo-redo / ⬆ export. Row **delete asks `confirm()`** first (undoable). `loadWorkbook` switches to the Roster view; Solve lives in the Groups view.
