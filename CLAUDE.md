...
- **OME OS shell** — the whole app is wrapped in a small platform shell: a fixed left **sidebar** (`.os-nav`, brand **OME OS** + JABSOM logo) lists **tools** from a `TOOLS` registry (`{id,name,icon,available}`), and each tool is a `.tool-panel` (`#tool-<id>`) in `.os-body`. **PBL Group Builder** (`#tool-pbl`) is the first/only real tool; `renderTools()`/`selectTool(id)` switch panels, the sidebar collapses to an icon rail (`#osCollapse`, persisted in `localStorage 'pbl.navCollapsed'`), and it becomes a top strip on narrow screens. **Adding a tool = one `TOOLS` entry + a `#tool-<id>` panel.** The print CSS still works because `#printRoster` stays a direct `body` child outside `.os`.
- **App shell** — within the PBL tool, two top-level tabs (`#appTabs`: **Roster** ↔ **Groups**) toggle `#view-roster` / `#view-groups` via `showView()`; a header **icon transport bar** (`.iconbtn`, inline-SVG, aria-labelled) holds example / open / paste / template / undo-redo / export, plus a **?** that opens a how-it-works dialog. Solve lives in the Groups view; `loadWorkbook` switches to Roster.
